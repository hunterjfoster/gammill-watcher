...
-------------------------------------
Reads the live data feeds behind https://gammill.com/preloved/ (two tabs of a
published Google Sheet, each fetched as CSV: one for individual pre-loved
machines, one for retrofit-package configurations) and:
...
# Discovered via the site's Network tab on 2026-09-26 - a second tab in the
# same spreadsheet, holding retrofit-package listings (statuses include
# "draft" for not-yet-available configurations, in addition to active/sold).
RETROFIT_CSV_URL = (
    "https://docs.google.com/spreadsheets/d/"
    "1x0S6gnlnMGPq0uXl2JhxdLIr86DzjIYiBG8Asg3kJPM/gviz/tq?tqx=out:csv&gid=2126555108"
)
...
def fetch_csv(url, timeout=45, attempts=3):
    """Pull one CSV feed, retrying on transient network errors."""
    last_error = None
...
def fetch_listings():
    """Pull both CSV feeds (pre-loved machines + retrofit packages) and
    return a combined list of row dicts, keyed by column name."""
    listings = []
    for url in (CSV_URL, RETROFIT_CSV_URL):
        text = fetch_csv(url)
...
def format_listing(listing):
    ...
    return (
        f"{kind}\n"
        f"  Machine Year: {listing.get('year', '?')}\n"
        f"  Machine Model: {listing.get('family', '').title()}\n"
        f"  Throat Depth: {throat}\"\n"
        f"  Table Length: {listing.get('table', '?')}\n"
...
def main():
    ...
    for listing in listings:
        ...
        if status != old_status:
            ...
            elif status == "active" and old_status in ("sold", "removed", "unknown", "draft"):
                # (Re-)listed
                new_active_listings.append(listing)
                log_rows.append(make_log_row(now, "Listed", lid, info))
